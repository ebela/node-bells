...
module.exports.Predictor = function(criteria) {
	...
	var Predictor = {
		...
		predict: function(now) {
			if (!_.isString(now) || now.length == 0) return null;
			var _today = moment();
			if (!now) now = _today;
			else now = moment(now);
			...
			// Match specific dates first
			if (Predictor._date[formattedNow]) return Predictor._date[formattedNow];

			// Match day-of-week predicates
			if (Predictor._dow[dow]) return Predictor._dow[dow];

			// If nothing else matched
			return Predictor._default;
		},
	};
